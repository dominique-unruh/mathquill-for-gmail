...
async function update_pic(img,ltx) {
  ...
  dataurl = await get_data_url(url);
...
async function copy_paste_pic(img) {
  var sel = document.getSelection();
  sel.removeAllRanges();
  var range = document.createRange();
  range.setStartBefore(img[0]);
  range.setEndAfter(img[0]);
  sel.addRange(range);

  // WARNING: looses alt, title attributes.
  document.execCommand("copy");
  document.execCommand("paste");

  // Hack to make sure GMail has converted the pasted pic
  await sleep(5000);
  
  // Copy&paste, so we need to fix the size again
  var sel = document.getSelection();
  ...
  var newImg0 = range.startContainer.childNodes[range.startOffset];
...
// TODO remove
async function put_data_url_img(img,url) {
  var dataurl = await get_data_url(url);
  console.log(dataurl);
  img.attr("src",dataurl);
};
...
// Adapted from https://stackoverflow.com/a/30407959
...
async function get_url_as_arraybuffer(url) {
  var response = await fetch(url);
  if (!response.ok) {
    console.error("Could not fetch "+url, response);
    return null;
  }
  var dataurl = await response.arrayBuffer();
  return dataurl;
}


async function get_data_url(url) {
  var response = await fetch(url);
  if (!response.ok) {
    console.error("Could not fetch "+url, response);
    return null;
